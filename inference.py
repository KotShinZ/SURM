...
def inference(model, dataloader, output_keys: List[str], device="cuda"):
    ...
    with torch.inference_mode():
        for i, (set_name, batch, global_batch_size) in enumerate(tqdm(dataloader)):
            ...
            # --- DEBUG: 初回のみキーを確認 ---
            if i == 0:
                print(f"[DEBUG] Keys returned by model: {list(preds.keys())}")
            # -------------------------------
...
def main():
    ...
    # --- 修正: logits も要求するように変更 ---
    output_keys = config.eval_save_outputs if config.eval_save_outputs else []
    # outputs がダメなら logits を試すために両方入れる
    target_keys = ["outputs", "logits"]
    ...
    # --- 修正: logits もチェック ---
    has_labels = "labels" in targets
    
    predictions_tensor = None
    if "outputs" in results:
        print("Found 'outputs' in results.")
        predictions_tensor = results["outputs"]
    elif "logits" in results:
        print("Found 'logits' in results. Using it as predictions.")
        predictions_tensor = results["logits"]
    else:
    ...
    if predictions_tensor is not None and has_labels:
        ...
        if not has_labels:
            print(" - 'labels' missing in targets.")
